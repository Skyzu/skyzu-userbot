...
async def text_to_speech(query):
    ...
    if message:
        ...
        return await edit_delete(
            query, "`Give a text or reply to a message for Text-to-Speech!`"
        )
...
CMD_HELP.update({"barcode": f"𝘾𝙤𝙢𝙢𝙖𝙣𝙙 `{cmd}barcode` <content>"})
